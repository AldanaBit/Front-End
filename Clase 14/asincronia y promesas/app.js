...
// PROBLEMA : no se puede "procesar el video" antes de "descargarlo"
// Usando callbacks nosotros manejamos cuando se debe invocar la funcion "procesaVideo"
...
const siMePortoBien = new Promise((resolve, reject) => {
  if (teHasPortadoBien) {
    ...
    resolve(telefono); // retornamos el objeto telefono
  } else {
    reject('te has portado mal');
  }
});

//4.3 Uso (consumo) de PROMISES (usamos las promesas)
...
const estudieBien = chequeaCalificacion(86);
...
async function pideTelefonoNuevo(comportamiento, calificacion) {
  try {
    ...
    // codigo que se ejecuta si se genera un error
    console.log(error);
  }
}
...
// link al video explicativo : https://youtu.be/HgqNstf4xg0?si=RnMSWb8arZqhiwli